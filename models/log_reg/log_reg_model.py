...
from models.log_reg.features import process_features, tf_idf

### FUNCTION DEFINITION ###


def log_reg_predict(training_set, validation_set, testing_set, cfg):
    """Predicts a connection with a Logistic Regression classifier.
    ...
    classifier = LogisticRegression(
        penalty=cfg["log_reg"]["penalty"],
        dual=cfg["log_reg"]["dual"],
        C=cfg["log_reg"]["C"],
        class_weight=cfg["log_reg"]["class_weight"],
        max_iter=cfg["log_reg"]["max_iter"],
        random_state=cfg["log_reg"]["random_state"],
    )
    ...
    predictions_log_reg = np.array(classifier.predict(test_features))
    ...
    if validation_set:
        ...
        return predictions_log_reg, score
    else:
